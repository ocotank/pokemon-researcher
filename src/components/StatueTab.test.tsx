...
async function createCollection(): Promise<Collection> {
  const [photos, spotShots] = await Promise.all([getAllStatuePhotos(), getAllSpotPhotos()]);
  return {
    ...
    spotPhotos: new Map(spotShots.map((photo) => [photo.spotId, photo])),
    panels: [],
    statueNames: new Map(),
...
describe('StatueTab', () => {
  ...
  it('全体写真の欄を立像の一番下に表示する', async () => {
    render(<StatueTab collection={await createCollection()} geo={geoWithoutPosition} />);

    const firstCard = screen.getAllByRole('article')[0];
    const rows = firstCard.querySelectorAll('.statues li');
    expect(rows[rows.length - 1].textContent).toContain('全体写真');
  });

  it('全体写真が保存済みなら撮り直すと表示する', async () => {
    await putSpotPhoto({ spotId: 'terrace-b1f', blob: new Blob(['spot']), takenAt: 1 });

    render(<StatueTab collection={await createCollection()} geo={geoWithoutPosition} />);

    const overall = document.querySelector('.statues li.overall');
    expect(overall?.textContent).toContain('撮り直す');
    expect(overall?.className).toContain('done');
  });
});
